...
var iron = false;
if ((process.env.IRON_CACHE_PROJECT_ID) && (process.env.IRON_CACHE_TOKEN)) {
  iron = require('iron-cache').createClient({ project: process.env.IRON_CACHE_PROJECT_ID, token: process.env.IRON_CACHE_TOKEN });
}
...
SMSglue.put = function(category, id, value, cb) {
  if (iron) {
    iron.put(category, id, { value: value }, cb);
  
  } else {
    var filename = os.tmpdir() + `/SMSglue-${id}.${category}`;
    fs.writeFile(filename, value, 'utf8', cb);
  }
}

SMSglue.get = function(category, id, cb) {
  if (iron) {
    iron.get(category, id, cb);
